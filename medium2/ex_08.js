...
function longestSentence(string) {
  let terminatingChars = ['!', '.', '?'];
  let sentenceArray = [];
  let startIndex = 0;

  for (let index = 0; index < string.length; index += 1) {
    if (terminatingChars.includes(string[index])) {
      sentenceArray.push(string.slice(startIndex, index + 1));
      startIndex = index + 2;
    }
  }
  
  let sentenceWordsArray = sentenceArray.map(sentence => sentence.split(' '))
                                        .sort((a, b) => b.length - a.length);
  
  console.log(sentenceWordsArray[0].join(' '));
  console.log(sentenceWordsArray[0].length);
...
// Further Exploration
// Assignment: Can you modify your solution to find the longest paragraph
//              (you can embed newlines in your string to mark paragraphs).
